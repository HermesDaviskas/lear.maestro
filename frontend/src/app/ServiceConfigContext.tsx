...
export async function loadServiceConfigFromDisk(): Promise<{
  ...
  try {
    const [servicesRes, serversRes] = await Promise.all([
      axios.get(
        `http://localhost:3000/api/jsonHandler?file=${settingsFilePath}&key=services`
      ),
      axios.get(
        `http://localhost:3000/api/jsonHandler?file=${settingsFilePath}&key=servers`
      ),
    ]);
    ...
    console.error("Failed to load service config from disk (server-side)", err);
    throw err;
  }
}
